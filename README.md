...
- **`normalized/editor_overrides.json`** — ручные правки из редактора схемы.
  Сегодня в файле заполнен ровно один ключ — `layout` с координатами; `stations`,
  `lines`, `edges` лежат пустыми объектами.

### 3.2. Решатель

`go-layout-solver/` (Go) читает всё перечисленное и пишет `normalized/fullGraph.json`.

**Главное, что надо знать перед правкой солвера:** ручная и автоматическая раскладки —
взаимоисключающие ветки (`main.go`), а не два прохода подряд. Сейчас в
`editor_overrides.json` лежат координаты **всех** станций схемы, поэтому берётся ветка
`ApplyLayoutOverrides`, а `ApplyLayout()` со всем алгоритмом холодного старта
(`layout_bootstrap.go`, `separation.go`, `graph.go`) **не выполняется вовсе**.
Правка этого кода не изменит картинку. Он запускается только при пустых оверрайдах.

Что реально влияет на геометрию сегодня:

1. координаты в `editor_overrides.json`;
2. проход `ApplyRingProjection` (`rings.go`) — подгонка формы каждого кольца по его
   станциям, проекция станций на форму и публикация форм в `ringShapes` файла
   `fullGraph.json` (рантайм форму не подбирает, только читает — см. `docs/QUALITY.md`).

Ручные формы колец (`ringShapes` в оверрайдах) поддержаны, но в текущем файле их нет —
формы подбираются автоматически, солвер печатает результат подгонки для каждого кольца.
...
normalized/         производные данные: fullGraph.json, quality_report.json, оверрайды
scripts/            extract_yandex_coords.ts (парсер схемы Яндекса),
                    deploy.sh (деплой на свой сервер),
...
вопрос — **происхождение данных метро** в `new_map_source/`.
