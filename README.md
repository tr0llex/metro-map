...
Сквозные тесты (Playwright, `e2e/`) проходят приложение так, как это делает
человек. `npm run e2e` сам собирает проект и поднимает `preview` — именно
прод-сборку, потому что service worker и precache в дев-режиме не существуют.
...
пара станций даёт маршрут со временем, временем прибытия, пересадкой и
перечнем станций по отрезкам; альтернативные варианты переключаются и
перестраивают маршрут; кнопка обмена меняет станции местами и пересчитывает;
service worker активируется, precache наполняется, и повторный заход **без
сети** поднимает приложение целиком со схемой; тема переключается, переживает
перезагрузку и тянет за собой `meta theme-color`.

Сквозная проверка во всех тестах: любая ошибка в консоли браузера и любой
неудачный сетевой запрос валят тест. Приложение, отдающее 200 с упавшим
воркером маршрутов, здесь красное — по коду ответа оно выглядело бы живым.
...
Стенд визуальной приёмки (`tools/visual-qa`) — про другое и живёт отдельно:
он снимает скриншоты настоящим Chromium в Docker и сравнивает картинки.
Сквозные тесты проверяют поведение, а не пиксели.
...
```bash
npx tsc -b && npm run lint && npx vitest run && npm run build && npm run check:bundle && npm run e2e
```
