 ...
 *
 * ВАЖНО: ранняя (до первого кадра) установка атрибута И цвета системной строки
 * продублирована в public/theme-init.js — иначе принудительно тёмная тема даёт
 * вспышку светлой на старте. Тот скрипт подключён в index.html как обычный
 * (блокирующий) <script src>, а не инлайном, чтобы боевому CSP не требовался
 * 'unsafe-inline' в script-src. При изменении ключа, значений или цветов
 * правьте оба места.
 */
