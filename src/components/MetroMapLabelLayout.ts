...
export const LABEL_W = {
   ...
   * под текстом — читается нормально). Поэтому наложение стоит впятеро дороже
   * перечёркивания, и оптимизатор никогда не разменивает одно на другое.
   */
  labelOverlap: 60000,
   ...
   *
   * Величина выведена из «бюджета обхода», а не подобрана. Уехать от линии
   * стоит detachedStep + detachedQuad·over², поэтому подпись согласится
   * отойти на over = sqrt((lineCrossFirst − detachedStep) / detachedQuad)
   * пикселей сверх допуска. Свободные от линий коридоры внутри Кольцевой
   * начинаются примерно в 55–60px от станции — при прежних 10000 бюджет был
   * ровно 50px и до них не дотягивал: раскладка видела коридор, но считала
   * его слишком дорогим. 12000 дают бюджет 59px, то есть ровно столько,
   * сколько нужно, чтобы коридор стал достижим (и ни пикселем больше:
   * с 13000 и выше результат уже не меняется).
   *
   * Порядок штрафов при этом сохранён: перечёркивание (12000) дешевле
   * накрытого кружка станции (24000) и впятеро дешевле наложения подписей
   * (60000). Разменивать наложения на перечёркивания по-прежнему нельзя.
   */
  lineCrossFirst: 12000,
 ...
 *
 * В центре сетка самая мелкая (шаг 3px): свободные от линий «щели» там узкие,
 * и на прежнем шаге 3–6px оптимизатор в них просто не попадал.
 *
 * Верхняя граница набора — не вкусовая: она обязана дотягиваться до бюджета
 * обхода, который задаёт lineCrossFirst (см. там же). Прежние 56px в центре
 * обрубали перебор РАНЬШЕ, чем штрафы говорили «дальше не надо»: подписи
 * «Театральной» и «Библиотеки им.Ленина» физически некуда было поставить,
 * хотя свободное место в 60–80px от станции существовало. Дальше 88px
 * набор расширять бессмысленно — квадратичный штраф за отрыв делает такие
 * позиции дороже перечёркивания, и результат перестаёт меняться.
 */
const labelRadiusOffsetsForZone = (r: number): number[] => {
  if (r < LABEL_CENTER_RADIUS) {
    return [
      12, 15, 18, 21, 24, 27, 30, 33, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88,
    ]
  }
  if (r < LABEL_MIDDLE_RADIUS) {
    return [12, 15, 18, 21, 25, 29, 33, 38, 43, 49, 56, 63, 70, 78, 86, 94]
  }
