 ...
 *
 * Короткий тап отдаётся наружу (`onStationTap`) и заполняет поля по правилу
 * «первый — Откуда, второй — Куда». Долгое нажатие открывает поповер с явным
 * выбором поля: он позиционируется рядом с точкой нажатия и закрывается по
 * Escape, клику мимо и ресайзу.
 */
export function useStationPickPopover(params: {
  ...
  const handleMapSelect = useCallback((
    ...
    if (!isLongPress) {
      onStationTapRef.current(id, name)
      return
    }
